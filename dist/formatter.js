...
function styleToCss(style) {
    const cssProperties = [];
    if (style.fontSize)
        cssProperties.push(`font-size: ${style.fontSize}`);
    if (style.fontFamily)
        cssProperties.push(`font-family: ${style.fontFamily}`);
    if (style.color)
        cssProperties.push(`color: ${style.color}`);
    if (style.backgroundColor)
        cssProperties.push(`background-color: ${style.backgroundColor}`);
    if (style.margin)
        cssProperties.push(`margin: ${style.margin}`);
    if (style.padding)
        cssProperties.push(`padding: ${style.padding}`);
    if (style.textAlign)
        cssProperties.push(`text-align: ${style.textAlign}`);
    if (style.lineHeight)
        cssProperties.push(`line-height: ${style.lineHeight}`);
    if (style.fontWeight)
        cssProperties.push(`font-weight: ${style.fontWeight}`);
    if (style.fontStyle)
        cssProperties.push(`font-style: ${style.fontStyle}`);
    if (style.textDecoration)
        cssProperties.push(`text-decoration: ${style.textDecoration}`);
    if (style.width)
        cssProperties.push(`width: ${style.width}`);
    if (style.height)
        cssProperties.push(`height: ${style.height}`);
    if (style.maxWidth)
        cssProperties.push(`max-width: ${style.maxWidth}`);
    if (style.border)
        cssProperties.push(`border: ${style.border}`);
    if (style.borderRadius)
        cssProperties.push(`border-radius: ${style.borderRadius}`);
    if (style.borderCollapse)
        cssProperties.push(`border-collapse: ${style.borderCollapse}`);
    return cssProperties.join('; ');
}
// 应用样式
function applyStyles(paragraphs) {
    const styles = {};
    let html = '';
...
// 格式化内容
export async function formatContent(content, theme = themeConfig.currentTheme) {
